...
export async function main(ns) {
  ...
  if (!Number.isFinite(ram))
    throw new Error("Must enter number of gb");
  const cost = ns.getPurchasedServerCost(ram);
  const confirm = await ns.prompt(`Are you sure you want to buy ${times} ${ram}Gb server for ${formatMoney(cost)} each, total ${formatMoney(times * cost)}`);
  if (!confirm) {
    return;
  }

...
function growTarget(ns, target, slots) {
  const factor = ns.getServerMaxMoney(target) / ns.getServerMoneyAvailable(target);
  ...
  ns.print(`grow. threads=${formatInteger(threadsLeft)}/${formatInteger(threadsNeeded)}`);
  const runtime = {
...
function runScript(ns, script, host, threads, target, delay = 0, loop = false) {
  ns.exec(FILES[script], host, threads, ...[target, "--delay", delay, loop ? "--loop" : null].filter((v) => v != null));
}
